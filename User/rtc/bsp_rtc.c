 ...
 * 调用  ：内部调用
 */
void Time_Auto_Regulate(SLNoiseSet* TimeCorrect)
{
...
//	  Time_Regulate(tm);pMsg->m_iTime[0]
//	  for(i=0;i<6;i++)
//	{
//    printf("\r\nTimeCorrect->m_SystemTimeValue[i] is %x", TimeCorrect->m_SystemTimeValue[i]);
//  } 
	  {	
			tm.tm_year =TimeCorrect->m_SystemTimeValue[0]+2000;  //对输入的年份进行修正
			tm.tm_mon  =TimeCorrect->m_SystemTimeValue[1];
			tm.tm_mday =TimeCorrect->m_SystemTimeValue[2];
			tm.tm_hour =TimeCorrect->m_SystemTimeValue[3];	
			tm.tm_min  =TimeCorrect->m_SystemTimeValue[4];
			tm.tm_sec  =TimeCorrect->m_SystemTimeValue[5];
    }
...
//	   uint8_t str[15]; // 字符串暂存  	
	   ...
	   printf("\r\n 今天：%0.2d-%0.2d-%0.2d %0.2d:%0.2d", tm->tm_year,tm->tm_mon,tm->tm_mday,tm->tm_hour,tm->tm_min);
